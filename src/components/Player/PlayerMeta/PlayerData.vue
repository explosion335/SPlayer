<template>
  <div :class="['player-data', settingStore.playerType, { center, light }]">
    ...
    <n-flex :align="center ? 'center' : undefined" size="small" vertical>
      ...
      <!-- 作曲 -->
      <div v-if="musicStore.playSong.type !== 'radio' && musicStore.playSong.composer" class="album">
        <SvgIcon :depth="3" name="Music" size="20" />
        <span class="name-text text-hidden">{{ musicStore.playSong.composer }}</span>
      </div>
      <!-- 作词 -->
      <div v-if="musicStore.playSong.type !== 'radio' && musicStore.playSong.lyricist" class="album">
        <SvgIcon :depth="3" name="Edit" size="20" />
        <span class="name-text text-hidden">{{ musicStore.playSong.lyricist }}</span>
      </div>
      <!-- 編曲 -->
      <div v-if="musicStore.playSong.type !== 'radio' && musicStore.playSong.arranger" class="album">
        <SvgIcon :depth="3" name="Music" size="20" />
        <span class="name-text text-hidden">{{ musicStore.playSong.arranger }}</span>
      </div>
      <!-- 監製 -->
      <div v-if="musicStore.playSong.type !== 'radio' && musicStore.playSong.producer" class="album">
        <SvgIcon :depth="3" name="Music" size="20" />
        <span class="name-text text-hidden">{{ musicStore.playSong.producer }}</span>
      </div>
      <!-- 专辑 -->
